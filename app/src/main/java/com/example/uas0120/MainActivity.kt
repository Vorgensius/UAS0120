...
import com.example.uas0120.INTENT_DETAIL
import com.example.uas0120.Eventsitem
import com.example.uas0120.LeaguesItem
import com.example.uas0120.DetailActivity
import com.example.uas0120.ServerCall
import com.example.uas0120.gone
import com.example.uas0120.invisible
import com.example.uas0120.visible
...
class MainActivity : AppCompatActivity(), MatchView {
    ...
    private fun getlistLeageu() {
        if (presenter.isNetworkAvailable(this)) {
            presenter.getSpinnerData(this, object : ServerCall {
    ...
    private fun initContainer() {
        ...
        recyclerview.adapter = AdapterMatch { posistionData ->
    ...
    private fun setDataToContainer(id: String, menu: Int) {
        ...
        if (presenter.isNetworkAvailable(this)) {
            if (menu == 1) {
                showLoading()
                presenter.getPrevMatch(this, id, object : ServerCall {
                    override fun onSuccess(response: String) {
                        if (presenter.isSuccess(response)) {
                            try {
                                if (presenter.isSuccess(response)) {
                ...
                showLoading()
                presenter.getNextMatch(this, id, object : ServerCall {
                    override fun onSuccess(response: String) {
                        if (presenter.isSuccess(response)) {
                            try {
                                if (presenter.isSuccess(response)) {
    ...
    private fun getListAdapter(): AdapterMatch? = recyclerview?.adapter as? AdapterMatch
}